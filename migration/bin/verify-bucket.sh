...
#
# Tiers, cheapest/weakest first. Each is a strictly separate claim; see the runbook
# section "what verification can and cannot detect" for what each one does and does not
# prove. Running a cheaper tier does not substitute for a more expensive one -- they
# check different things, not the same thing at different confidence levels.
#
...
#          Does NOT detect: a defect confined to objects outside the sample. States a
#          numeric detection-probability bound for a given defect rate; see runbook.
#
