...
#                        catch-up pass (see runbook step "final gap copy").
#
...
# It must never delete anything on the destination -- see runbook "why copy, not sync".
#
...
do_pass() {
  ...
  # separate checkpoint file to lose or corrupt (see runbook "resumability").
