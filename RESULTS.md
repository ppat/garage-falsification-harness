...
| ------ | --------- | ------------ |
| H1 — RSS/LIST at scale | **PASSED — all three re-cut gates** | Completed to 3,439,460 (the full production object count); re-cut criteria (anon RSS, LIST p99, p99 scaling shape) pre-registered on #3611 before 2M/3.44M existed, all three PASS; see limitations below |
...
- **The sub-3KB inline path is 95.4% of this estate's objects, and this file cannot yet report
  on it.** Garage stores any object under `INLINE_THRESHOLD` (3072 B) inline in LMDB — no
...
**This test ran twice. The first attempt stalled at 500k for a reason directly relevant to
H2 (below); a second, complete run reached all five checkpoints and is what this section's
verdict is based on** — see "Verdict" below and `RESULTS-H1-completion.md` for the complete
run's full account, checkpoint table, and the re-cut criteria this verdict is judged against.
The first attempt's own numbers are kept here, not deleted, because the stall itself is
load-bearing evidence for H2.

**First attempt — checkpoints reached**:
...
**A new sizing input worth recording (from the first attempt; superseded by a firm number
below)**: `metadata_dir` runs roughly **1.6–2.6 KiB/object** across the measurements this
attempt happened to take. Extrapolated to 3.44M objects, that's **~5.6–9 GB of
`metadata_dir`** — in the same ballpark as the completed run's measured 6.67 GB, below.

**The original kill criteria (RSS > 6 GiB at 3.44M; p99 LIST > 1s; superlinear 1M→3.44M RSS
growth) were re-cut before the 2M/3.44M checkpoints existed** — see
`RESULTS-H1-completion.md`'s "Verdict against the three pre-registered kill criteria" for the
full re-cut criteria (C1/C2/C3), why the re-cut was necessary (MinIO's RSS is ~88% anonymous
and OOM-relevant; Garage's is ~98% reclaimable page cache, so judging both on total RSS would
have failed Garage for using memory that was available), and the pre-registration citation
(issue #3611, comment 5270927832).

**Verdict: PASSED — all three re-cut gates**, evaluated at the full 3,439,460-object
checkpoint: **C1** anonymous RSS 5.7 MiB (≤512 MiB and ≤ MinIO's 480.2 MiB, PASS, ~84× margin);
**C2** LIST p99 271.7 ms (≤1000 ms and ≤2× MinIO's 347.9 ms, PASS, below MinIO outright);
**C3** p99 growth 1M→3.44M sub-linear (2.32× for 3.44× objects, exponent ≈0.68, PASS). Full
verdict, checkpoint table, and the limitations that must travel with this result (coordinated
omission, n=200 per checkpoint, constrained-memory regime never exercised) are in
`RESULTS-H1-completion.md` — do not cite the PASS without those limitations.
...
**not** extend to the inline/sub-3KB object path — that gap is what H4-D exists to close.
