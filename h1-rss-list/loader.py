        ...
        if target <= next_index:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
                ...
                while i < target:
                    ...
                    # Incremental checkpoint: persist a conservative safe floor (the lowest
                    # still-outstanding index, or next_index if nothing is in flight) every
                    # ~5s, not just at the end of the whole write phase. A single-checkpoint
                    # write phase can take tens of minutes; without this, an interruption
                    # partway through (crash, external kill) loses the *entire* phase's
                    # progress on resume, not just the tail -- observed for real when this
                    # run's orchestrator process started being stopped by something outside
                    # this session every few minutes (see RESULTS-H1-completion.md). The
                    # floor is conservative (may be behind true completion by up to
                    # workers*4 objects) but always safe: PUTs are idempotent, so resuming a
                    # little early just re-writes a handful of already-done objects.
                    now = time.monotonic()
                    if now - last_persist >= 5:
                        safe_floor = min(futs.values()) if futs else next_index
                        json.dump({"next_index": safe_floor}, open(state_path, "w"))
                        last_persist = now
                # drain remaining
